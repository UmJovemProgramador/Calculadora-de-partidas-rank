var readline = require('readline');

var rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout
});

function calcularSaldoRank() {
    function processarRodada() {
        rl.question('Digite o número de vitórias: ', function(vitorias) {
            rl.question('Digite o número de derrotas: ', function(derrotas) {
                vitorias = parseInt(vitorias);
                derrotas = parseInt(derrotas);

                var saldoVitorias = vitorias - derrotas;

                var nivel;
                if (vitorias < 10) {
                    nivel = "Ferro";
                } else if (vitorias >= 11 && vitorias <= 20) {
                    nivel = "Bronze";
                } else if (vitorias >= 21 && vitorias <= 50) {
                    nivel = "Prata";
                } else if (vitorias >= 51 && vitorias <= 80) {
                    nivel = "Ouro";
                } else if (vitorias >= 81 && vitorias <= 90) {
                    nivel = "Diamante";
                } else if (vitorias >= 91 && vitorias <= 100) {
                    nivel = "Lendário";
                } else {
                    nivel = "Imortal";
                }

                console.log("O Herói tem um saldo de vitórias de " + saldoVitorias + " está no nível de " + nivel);

                rl.question('Deseja calcular para outro jogador? (Digite "y" para continuar): ', function(resposta) {
                    if (resposta.toLowerCase() === 'y') {
                        processarRodada();
                    } else {
                        rl.close();
                    }
                });
            });
        });
    }

    processarRodada();
}

calcularSaldoRank();
